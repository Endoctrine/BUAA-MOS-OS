...
int sys_env_destroy(u_int envid) {
	...
	//[shell_ext] printk("[%08x] destroying %08x\n", curenv->env_id, e->env_id);
	env_destroy(e);
	return 0;
}
...
int sys_ipc_try_send(u_int envid, u_int value, u_int srcva, u_int perm) {
	...
	if (srcva != 0) {
		...
		//try(sys_mem_map(0, srcva, e->env_id, e->env_ipc_dstva, perm));
		Pte *pte;
...
int sys_write_dev(u_int va, u_int pa, u_int len) {
	...
	u_int kseg1_va = pa | 0xa0000000;
	if(0x10000000 <= pa && pa + len <= 0x10000000 + 0x20){
		memcpy((void *)kseg1_va, (void *)va, len);
	}else if(0x13000000 <= pa && pa + len <= 0x13000000 + 0x4200){
		memcpy((void *)kseg1_va, (void *)va, len);
	}else if(0x15000000 <= pa && pa + len <= 0x15000000 + 0x200){
		memcpy((void *)kseg1_va, (void *)va, len);
	}else{
		return -E_INVAL;
	}
...
int sys_read_dev(u_int va, u_int pa, u_int len) {
	...
	u_int kseg1_va = pa | 0xa0000000;
	if(0x10000000 <= pa && pa + len <= 0x10000000 + 0x20){
		memcpy((void *)va, (void *)kseg1_va, len);
	}else if(0x13000000 <= pa && pa + len <= 0x13000000 + 0x4200){
		memcpy((void *)va, (void *)kseg1_va, len);
	}else if(0x15000000 <= pa && pa + len <= 0x15000000 + 0x200){
		memcpy((void *)va, (void *)kseg1_va, len);
...
void do_syscall(struct Trapframe *tf) {
	...
	arg4 = *((u_int *)(((void *)tf->regs[29]) + 16));
	arg5 = *((u_int *)(((void *)tf->regs[29]) + 20));
	/* Step 5: Invoke 'func' with retrieved arguments and store its return value to $v0 in 'tf'.
	 */
	/* Exercise 4.2: Your code here. (4/4) */
	int return_value = func(arg1, arg2, arg3, arg4, arg5);
	tf->regs[2] = return_value;
}
