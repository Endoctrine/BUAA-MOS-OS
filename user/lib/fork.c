...
static void __attribute__((noreturn)) cow_entry(struct Trapframe *tf) {
	...
	Pte *pte = (void *)vpt + (VPN(va) * 4);
	perm = (*pte) & 0x00000fff;
	if(!(perm & PTE_COW)){
		user_panic("not copy on write.");
	}
	...
	Pte *temp_pte;
	panic_on(syscall_mem_alloc(0, UCOW, perm));
	...
	syscall_mem_map(0, UCOW, 0, va & 0xfffff000, perm);
...
static void duppage(u_int envid, u_int vpn) {
	...
	syscall_mem_map(envid, addr, 0, addr, perm);
}
...
int fork(void) {
	...
	for(int i = 0; i < 1024; i++){
		if(*((Pde *)vpd + i) & PTE_V){
			for(int j = i * 1024; j < i * 1024 + 1024 && j < USTACKTOP / BY2PG; j++){
				if(*((Pte *)vpt + j) & PTE_V){
					duppage(child, j);
				}
			}
